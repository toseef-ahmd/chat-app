...
import { FetchUserFoundResponse } from './dto/user.dto/specific-user.dto';
...
export class UserController {
  ...
  async create(
    ...
    return {
      ...
      links: GetHyperLinks(Routes.User, Methods.create).map(
        (link) => new LinkDto(link.href, link.method),
      ),
      //data: user
  ...
  async findAll(): Promise<ApiResponse<Array<IUser>>> {
    ...
    return {
      ...
      message:
        users.length > 0 ? 'Users fetched successfully' : 'No users were found',
  ...
  async update(
    ...
    return {
      ...
      links: GetHyperLinks(Routes.User, Methods.update).map(
        (link) => new LinkDto(link.href, link.method),
      ),
      data: user,
    } as unknown as UserUpdateResponseDto;
